...
func (s *Suite) TestItems() {
	...
	s.Run("Deve retornar uma mensagem de erro quando não houver items cadastrados", func() {
		// // instancia factory de items
		factory := Item_test_factory{}
	...
	s.Run("Deve ser capaz de listar todos os items", func() {
		// Create a New Request
		req, _ := http.NewRequest("GET", "/items", nil)
	...
	s.Run("Deve ser capaz de deletar um item", func() {
		// instancia factory de items
		factory := NewItemTestFfactory()
